...
| **`dotfiles-secrets`** | every *identifier* — internal hosts, VPN endpoint, account names, ssh key paths, and the 41-path secrets manifest | keep private. Identifiers only, still no passwords. |
...
# 6. themes (not in this repo - they are their own git clones, 662MB)
omarchy-theme-install <url>   # per theme in config/.config/omarchy/current/theme.name
...
**mpvpaper** and **hyprlock** are *not* custom builds — stock AUR `1.9-1` and stock Arch
...
- `~/.local/bin/lock-shader` is the lock entry point: it enables
  `misc:session_lock_xray` so the live shader shows *through* hyprlock, and kills
...
|---|---|
| `~/.config/omarchy/themes/` (662MB) | Each theme is its own git clone. `omarchy-theme-install`. |
| `~/.config/omarchy/current/theme/` (19MB) | Generated from `theme.name` on theme switch. |
...
| `graphify`, `graphify-mcp`, `hyprshade` symlinks | Install with `uv tool install graphifyy` and `pipx install hyprshade`. Note hyprshade is *also* a pacman package — duplicate, pick one. |
